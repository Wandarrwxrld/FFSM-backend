...
/**
 * Returns the config array, building it only once per request (cached in
 * a static variable). Call this from anywhere that needs config values —
 * never `require` this file a second time in the same request, since
 * re-executing it would try to redefine ffms_load_env()/ffms_env() above
 * and crash with a fatal "Cannot redeclare function" error.
 */
function ffms_config(): array
{
    static $config = null;
    if ($config !== null) {
        return $config;
    ...
    $config = [
        ...
        'mail' => [
            'resend_api_key' => ffms_env('RESEND_API_KEY', ''),
